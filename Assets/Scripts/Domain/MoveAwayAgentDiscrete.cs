...
public class MoveAwayAgentDiscrete : Agent
{
    ...
    [SerializeField] private bool enableMoveCloser = false;
    [SerializeField] private bool enableSmoothTurn = true;
    [SerializeField] private float changingAngleRewardMaxFactor = 0.1f;

    [SerializeField] private bool randomizeChangingAngleInterval = false;
    [SerializeField] private float randomizeChangingAngleIntervalMin = 0;
    [SerializeField] private float randomizeChangingAngleIntervalMax = 4;

    [SerializeField] private bool enableChangingAngleInterval = false;
    [SerializeField] private float changingAngleInterval = 1.0f;
    [SerializeField] private float changingAngleIntervalTimer = 0;
    [SerializeField] private bool moveRandomOnFreeSpace = false;
    [SerializeField] private bool forceMoveRandomOnFreeSpace = false;
    [SerializeField] private int randomMoveActionMin = 0;
    [SerializeField] private int randomMoveActionMax = 16;
    ...
    private float previousAc0 = 0;
    public override void OnActionReceived(ActionBuffers actions)
    {
        float ac0 = actions.DiscreteActions[0]; // 16 arah

        if (moveRandomOnFreeSpace || forceMoveRandomOnFreeSpace)
        {
            if (nearestObject == null || forceMoveRandomOnFreeSpace)
            {
                ac0 = (16 + Random.Range(randomMoveActionMin, randomMoveActionMax)) % 16;

                if (enableChangingAngleInterval)
                {
                    if (changingAngleIntervalTimer < 0)
                    {
                        if (randomizeChangingAngleInterval)
                        {
                            changingAngleInterval = Random.Range(randomizeChangingAngleIntervalMin, randomizeChangingAngleIntervalMax);
                        }
                        changingAngleIntervalTimer = changingAngleInterval;
                        previousAc0 = ac0;
                    }
                    else
                    {
                        changingAngleIntervalTimer -= Time.deltaTime;
                        ac0 = previousAc0;
                    }
                }
            }
        ...
        float normalizedAc0 = ((1.0f + (ac0 / 16)) % 1.0f) * 2 * Mathf.PI;
    ...
    public void SetMoveCloser(bool enable)
    {
        enableMoveCloser = enable;
    }
}
